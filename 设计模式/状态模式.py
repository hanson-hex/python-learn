...
class Context(metaclass=ABCMeta):
    ...
    def addState(selfself, state):
        if (state not in self.__states):
            self.__states.append(state)

    def changeState(selfself, state):
        if (state is None):
            return False
    ...
    def getState(selfself):
        return self.__curState

    def _setStateInfo(selfself, stateInfo):
        self.__stateInfo = stateInfo
        for state in self.__states:
            if( state.isMatch(stateInfo) ):
                self.changeState(state)

    def _getStateInfo(selfself):
...
class State:
    ...
    def isMatch(selfself, stateInfo):
        "状态的属性stateInfo是否在当前的状态范围内"
        return False

    @abstractmethod
    def behavior(selfself, context):
        pass
...
class Water(Context):
    ...
    def getTemperature(selfself):
        return self._getStateInfo()

    def setTemperature(selfself, temperature):
        self._setStateInfo(temperature)

    def riseTemperature(selfself, step):
        self.setTemperature(self.getTemperature() + step)

    def reduceTemperature(selfself, step):
        self.setTemperature(self.getTemperature() - step)

    def behavior(selfself):
        state = self.getState()
